...
上述命令都應以零結束。2026-08-21 的觀測結果是 pgTAP `123` tests、Vitest `27` files／`283` tests、ESLint 無錯誤、Vite build 成功，以及 DB lint 回報 `No schema errors found`；隨測試演進，總數可能改變。Supabase CLI 的 DB gate 依賴 Supabase config 與 container runtime，不需要 Vite 的 `.env.local`。
